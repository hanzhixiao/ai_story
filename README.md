...
- 左侧历史对话栏上下一分为二，下面部分新增一个我的故事栏
 历史对话栏和我的故事栏都是可以滑动的列表
- 在AI返回的内容框右下角增加一个圆形的图标，点击会展开一个选项列表，有一个选项是添加为故事

### 后端

- 后端有/stories的系列接口，可以供前端请求

## v0.4.1 Features

### 前端

- 圆形的图标展示在AI返回的内容框外侧，右下角

## v0.4.2 Features

### 前端

- 点击添加为故事后，不直接调用保存故事的接口，而是在前端页面展示一个可以编辑的浮层
    - 浮层样式简洁，右上角有关闭选项和保存选项
    - 用户可以在浮层内编辑刚刚想要保存为故事的内容
    - 点击保存才会调用保存为故事的接口
    - 在保存时，前端需要通过某种算法为当前文本计算一个独一无二的特征值，传递给后端接口，根据后端的返回，展示保存成功或者是有重复故事或者保存失败

### 后端

- 后端不仅要保存故事的基本内容，同时为了避免重复内容多次保存，需要按照同样的方式为当前文本计算一个独一无二的特征值
    - 将计算的特征值和前端传入的特征值进行比较
        - 如果不相同，说明请求被修改，返回错误
        - 如果相同，则进行后续流程
    - 查找这个用户的所有故事的特征值，比较是否有相同的
        - 如果没有，说明是全新的文本，则进行保存
        - 如果有，则告知前端是重复文本
